...
const setOptionsAndReturnOpenGraphResults = function (options, callback) {
  ...
  if (validate.returnInputUrl) {
    ...
    requestAndResultsFormatter(options, function (error, results, response) {
      if (results) {
        return callback(false, { data: results, success: true }, response);
      } else {
        if (error && (error.code === 'ENOTFOUND' || error.code === 'EHOSTUNREACH')) {
          return callback(true, { error: 'Page Not Found', success: false, errorDetails: error }, response);
        } else if (error && error.code === 'ETIMEDOUT') {
          return callback(true, { error: 'Time Out', success: false, errorDetails: error }, response);
        } else if (error && error === 'Must scrape an HTML page') {
          return callback(true, { error: 'Must scrape an HTML page', success: false, errorDetails: error }, response);
        } else {
          return callback(true, { error: 'Page Not Found', success: false, errorDetails: error }, response);
        }
      }
    ...
    return callback(true, { error: 'Invalid URL', success: false, errorDetails: 'Invalid URL' }, null);
  }
};
  ...
  request(options, function (error, response, body) {
    if (error) {
      ...
      keys.forEach(function (key) {
        ...
        const content = meta[key].attribs.content;
