...
var requestAndResultsFormatter = function (options, callback) {
  ...
  fetch(options.url, options).then(feedback => {
    // handle response codes 4xx and 5xx
    if (feedback
    && feedback.status
    && (feedback.status.toString().substring(0, 1) === '4' || feedback.status.toString().substring(0, 1) === '5')) {
      return callback({error: 'Server Has Ran Into A Error'}, null, feedback);
    }

    // convert response to utf-8 body
    return feedback.textConverted().then(formatBody => {
      var ogObject = extractMetaTags(formatBody, options);
      if (options.withCharset) {
        ogObject.charset = charset(feedback, formatBody, peekSize);
      }
      return callback(null, ogObject, feedback);
    }).catch(err => {
      return callback(err, null, feedback);
    });
  }).catch(err => {
