...
const setOptionsAndReturnOpenGraphResults = async (options) => {
  ...
  const validate = utils.validate(options.url, options.urlValidatorSettings);
  ...
  try {
    ...
    const results = await requestAndResultsFormatter(gotOptions, options.downloadLimit, options.peekSize);
    return results;
  } catch (exception) {
