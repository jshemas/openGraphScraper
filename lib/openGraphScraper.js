  ...
  return request.get(requestUrl, options)
    .then((response) => {
      ...
      if (response && response.statusCode && (response.statusCode.toString().substring(0, 1) === '4' || response.statusCode.toString().substring(0, 1) === '5')) {
        throw new Error('Server has returned a 400/500 error code', 500);
      }
      if (options.runChar) {
...
const setOptionsAndReturnOpenGraphResults = (options, callback) => {
  ...
  if (validate.returnInputUrl) {
    ...
    return requestAndResultsFormatter(options)
      .then((results) => callback(false, {
        data: results.ogObject, success: true, requestUrl: options.url,
      }, results.response))
      .catch((error) => {
        if (error && (error.code === 'ENOTFOUND' || error.code === 'EHOSTUNREACH')) {
          return callback(true, {
            error: 'Page Not Found', success: false, requestUrl: options.url, errorDetails: error,
          });
        } if (error && error.code === 'ETIMEDOUT') {
          return callback(true, {
            error: 'Time Out', success: false, requestUrl: options.url, errorDetails: error,
          });
        }
        return callback(true, {
          error: 'Page Not Found', success: false, requestUrl: options.url, errorDetails: error,
