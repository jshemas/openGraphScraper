...
/**
 * performs the request and formats the results
 *
...
exports.requestAndResultsFormatter = async (gotOptions, ogsOptions) => {
  ...
  return got(gotOptions)
    .then((response) => {
      ...
      const char = charset.find(response.headers, formatBody, ogsOptions.peekSize) || chardet.detect(formatBody);
      ...
      const ogObject = extractMetaTags(formatBody, ogsOptions);

      if (!ogsOptions.onlyGetOpenGraphInfo) {
        ogObject.charset = charset.find(response.headers, formatBody, ogsOptions.peekSize);
      }
