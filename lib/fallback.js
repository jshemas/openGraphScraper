...
const fallback = (ogObject, options, $) => {
  ...
  // locale fallback
  if (!ogObject.ogLocale && $('html').attr('lang') && $('html').attr('lang').length > 0) {
    ogObject.ogLocale = $('html').attr('lang');
  } else if (!ogObject.ogLocale && $('head > meta[itemprop="inLanguage"]').attr('content') && $('head > meta[itemprop="inLanguage"]').attr('content').length > 0) {
